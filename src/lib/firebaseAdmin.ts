...
if (!admin.apps.length) {
  ...
  if (serviceAccount && !initializationError) {
    try {
     ...
     initializationError = "Could not form service account credentials for Firebase Admin SDK. Critical environment variables are likely missing.";
...
export function getAdminInitializationError(): string | null {
  if (initializationError) {
    return `Firebase Admin SDK Initialization Failed: ${initializationError}. Server-side Firebase features will be unavailable.`;
  }
  if (!firebaseAdminApp) {
    return "Firebase Admin App (firebaseAdminApp) is undefined. Initialization likely failed silently or was not attempted. Server-side Firebase features will be unavailable.";
  }
  if (!(typeof firebaseAdminApp.INTERNAL === 'object' && firebaseAdminApp.INTERNAL !== null)) {
    return "Firebase Admin App's INTERNAL object is undefined. SDK might not be fully functional (credentials or setup issue). Server-side Firebase features will be unavailable.";
  }
